...
class AudioTranscriber:
    ...
    def _get_file_hash(self, file: Path) -> str:
        """Generate SHA hash of file contents"""
    ...
    def _get_cache_path(self, audio_path: str) -> Path:
        """Get the cache file path for a given audio file using SHA hash of contents"""
        audio_file = Path(audio_path)
        cache_dir = Path.home() / ".cache" / "audio_search"

        # Generate SHA hash of file contents
        sha_hash = self._get_file_hash(audio_file)

        # Include backend name in cache to avoid conflicts
        return cache_dir / f"{sha_hash}.{self.backend_name}.transcription.json"
...
class TranscriptionSearcher:
    ...
    def _build_word_index(self):
        ...
        for segment in self.transcription.get("segments", []):
            ...
            if "words" in segment:
                ...
                self.words.append({"word": text, "start": segment["start"], "end": segment["end"]})
    ...
    def _char_to_word_indices(
        self, word_texts: List[str], start_char: int, end_char: int
...
Examples:
  ...
  %(prog)s audio.mp3 --regex --search "\\b(hello|hi)\\b" --headless
    ...
    # Required arguments
    parser.add_argument("audio_file", help="Path to the audio file (mp3, mp4, wav, etc.)")
    ...
    parser.add_argument("--search", "-s", help="Search query (required in headless mode)")
    ...
    try:
        ...
        if torch.cuda.is_available():
            print(
                f"✓ NVIDIA GPU available: {torch.cuda.get_device_name(0)}",
                file=sys.stderr,
            )
        elif hasattr(torch, "hip") and torch.hip.is_available():
            print("✓ AMD GPU (ROCm) available", file=sys.stderr)
        else:
    ...
    print(
        f"Transcription ready. Total segments: {len(transcription.get('segments', []))}",
        file=sys.stderr,
    )
    print(
        f"Total duration: {format_timestamp(transcription.get('duration', 0))}",
        file=sys.stderr,
    )
